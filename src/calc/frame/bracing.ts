...
export interface BracingInput {
  ...
  strutTube?: StrutTube;
  /**
   * Труба вертикальных связей — по умолчанию 80х3 (L85), как в "22316"
   * и "22318" (оба пролёт > 12 м). "22069" (пролёт 12 м, k=1,0) дал
   * первый пример 120х3 (L83) — при этом горизонтальные связи там же
   * остались на 80х3, так что замена не сквозная. Полная реконструкция
   * формулы C96 с этой трубой сошлась с ведомостью до 10-го знака
   * (2,8596735037793 т), поэтому это не совпадение — но неясно, тот же
   * ли это ручной выбор конструктора, что и для трубы распорок, или
   * есть правило (например, по пролёту) — см. вопрос расчётчику.
   */
 ...
 *     +  4 × √(высота²    + шаг²) × масса_трубы_верт × 1,1   вертикальные связи
 *     + ручная добавка
 ...
 *
 * Сверка: "22316" (18×30, h5, шаг 4,5, 8 рам, распорки 80х3, верт.
 * связи 80х3, добавка 0,432 т, окно 30 м → обрамление 31,5 м, периметр
 * 65 м) → 3,1503 т / 1,0390 т / 2,112 т. Файл в этом месте хранит
 * периметр как 62 м (окно 30 м как есть, без округления до кратного
 * шагу) — расчётчик подтвердила, что это ошибка файла (вопрос 01),
 * поэтому с ним мы расходимся здесь намеренно, см. computeProject.test.ts.
 * "22318" (15×24, h5, шаг 4, 7 рам, распорки 60х3, верт. связи 80х3,
 * добавка 0,795 т, окон нет) → 2,4974 т / 0,7215 т / 1,666 т.
 * "22069" (12×20, h6, шаг 5, 5 рам, k=1,0 — не авто-подобранный k=0,8,
 * см. вопрос расчётчику про "22330", ту же ситуацию встречали раньше,
 * распорки 80х3, верт. связи 120х3, добавка 0,436 т, периметр окон
 * 64 м) → 2,8596735037793 т, совпадение с ведомостью до 10-го знака.
 *
 * ЧТО ЗАДАЁТСЯ ВРУЧНУЮ И ПРАВИЛА ДЛЯ ЭТОГО НЕТ:
 *   · слагаемое 0,432 / 0,795 / 0,436 т — зависимости от габаритов не видно;
 *   · размер трубы распорок (80х3 / 60х3) и трубы вертикальных связей
 *     (80х3 / 120х3 в "22069") — возможно, тот же ручной выбор
 *     конструктора, что и для распорок, а не правило по габаритам;
 *   · их количество (3 во всех трёх проектах).
 *
...
export function computeBracing(input: BracingInput): BracingTakeoff {
  ...
  const vertTube = input.verticalBraceTube ?? BRACE_TUBE;
  const vertBraceTube_t_per_m = TUBE_MASS_t_per_m[vertTube];
  ...
  const tubeParts = [
    ...
    },
    {
      ...
      mass_t: 4 * vertBraceLength_m * vertBraceTube_t_per_m * BRACE_ALLOWANCE,
    },
