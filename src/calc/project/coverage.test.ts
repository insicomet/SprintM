...
describe("покрытие климатического справочника", () => {
  ...
  it("помечает «требует проверки» ровно 49 городов на краю таблиц", () => {
    const flagged = results.filter((r) => r.project.requiresCheck);
    // 47 — это весь край справочника: Камчатка, Сахалин, Курилы, Кольский
    // полуостров, Воркута, Черноморское побережье, Дагестан. Плюс 2 —
    // казахстанские «Кокшетау» и «Караганда (СН, уклон 6°)»: их сочетание
    // с/в тоже не просчитано в банке сечений. Число здесь не «сколько
    // получилось», а граница: если оно выросло, значит приблизительным
    // стал считаться кто-то ещё.
    expect(flagged.length).toBe(49);
  ...
  it("остальные города считаются точно, без пометки", () => {
    ...
    expect(exact.length).toBe(settlements.length - 49);
