...
export interface SettlementClimate {
  ...
  wind: {
    ...
    w0Kpa: number | null;
    /**
     * Районы, на границе которых стоит город, — заполнено только там,
     * где сам район не проставлен. Три таких города: Дербент и Избербаш
     * между IV и V, Багратионовск между I, II и III (ответ расчётчика
     * на вопрос 04). Расчёт берёт из этого списка самый тяжёлый район и
     * помечается «требует проверки».
     */
    borderRegions?: readonly string[];
  };
}
