...
export interface GirtAutoSelectionInput {
  ...
  w0_kPa: number;
  terrain: TerrainType;
  /** γn — коэффициент ответственности (Лист1!B3, тот же смысл, что и bankK для рамы). */
  gammaN: number;
 ...
 * местности «В», γn=0,8, профлист без утепления, высота профиля
 * зажата на 145 мм): для обеих зон профиль, число рядов, число
 * кронштейнов и масса профиля совпали с ведомостью день в день —
 * "[]ПП 145x45x1,5" 448,8288 кг (угловая), "[]ПП 145x45x1,2"
 * 359,1504 кг (рядовая), см. selectGirt.test.ts. Шаг ригелей внутри
 * этого алгоритма (1330 мм для обеих зон) отличается от ведомости
 * (1370/1380 мм) — но масса при любом шаге в этом диапазоне одна и та
 * же (число рядов не меняется), так что это другая точка того же
 * минимума, а не ошибка; при равенстве массы код берёт меньший шаг из
 * плато, файл — какой-то другой (не восстановленное правило
 * разрешения ничьих, не критично).
 *
 ...
 *  — фильтры «Вид профиля»/«Тип сечения»/«Марка материала»/«Без стоек»
 *    (ручные тумблеры Лист1!V89:W94/V97:W99/V102:W105/W107) — сейчас
 *    не ограничиваем: у нас нет строк «+ стойки» вовсе (только базовый
 *    набор «несушки»), а вид/тип сечения/материал в проверенном примере
 *    были разрешены все нужные, так что фильтр было не на чем проверить.
 */
export function selectGirtProfile(input: GirtAutoSelectionInput): GirtAutoSelectionResult | null {
  ...
  for (let step = STEP_MIN_mm; step <= STEP_MAX_mm; step += STEP_INCREMENT_mm) {
    ...
    for (const row of candidates) {
      // «Пред момент» несушки — уже готовое значение с применённым по
      // умолчанию коэффициентом использования (row.к_т_исп_по_умолчанию),
      // а не «сырой» момент профиля: домножать на него ещё раз нельзя,
      // иначе несущая способность занижается вдвое (на «Благовещенске»
      // из-за этого `[]ПП145x45x1,5` ошибочно не проходил проверку —
      // реальный коэфф. использования там 0,998, впритык, но проходит).
      // Раскладываем обратно на «сырой» момент только если задан ручной
      // коэффициент использования вместо коэффициента по умолчанию.
      const limitMoment = input.utilizationOverride
        ? (row.пред_момент / row.к_т_исп_по_умолчанию) * input.utilizationOverride
        : row.пред_момент;
